public class AVLTree {
	...
	public void makeAVL() throws Exception {
		...
		while (itr.hasNext()) {
			newTree.insert(itr.currentNode.data);
			itr.next();
		}
	...
	public boolean isAVL() {
		...
		while (itr.hasNext()) {
			try {
				int tiltDegree = AVLNode.tiltDegree(itr.currentNode);
				if (tiltDegree<-1||tiltDegree>1) return false;
				itr.next();
			} catch(Exception e) {
			}
		}
	...
	public static boolean same(AVLTree t1, AVLTree t2) {
		...
		AVLTreeIterator itr = (AVLTreeIterator) t1.findMin();
		while (itr.hasNext()) {
			try {
				AVLTreeIterator t1int2 = (AVLTreeIterator) t2.find(itr.currentNode.data);
				if (t1int2==null) return false;
				AVLNode a = itr.currentNode;
				AVLNode b = t1int2.currentNode;
				while (a!=t1.root && b!=t2.root) {
					a = a.parent;
					b = b.parent;
				}
