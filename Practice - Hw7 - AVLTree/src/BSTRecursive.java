...
public class BSTRecursive {
	...
	public int tiltDegree(BSTNode n) {
		if (n==null) {
			return 0;
		}
		return getHeight(n.left)-getHeight(n.right);
	}
	public boolean checkAVL(BSTNode n) {
		if (n==null) {
			return true;
		}
		boolean left = checkAVL(n.left);
		boolean right = checkAVL(n.right);
		return left && right && (Math.abs(tiltDegree(n)) <= 1);
	}
